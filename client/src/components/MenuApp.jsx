...
class MenuApp extends React.Component {
  ...
  findUniqueMenuTypes() {
    axios
      ...
      //.get(`http://localhost:3005/api/menus/${this.state.rest_id}`)
      .get(`http://13.57.201.108:9003/api/menus/${this.state.rest_id}`)
  ...
  filterDatabyMenuType() {
    axios
      // .get(`http://127.0.0.1:9003/api/menus/${this.state.rest_id}`)
      //.get(`http://localhost:3005/api/menus/${this.state.rest_id}`)docker 
      .get(`http://13.57.201.108:9003/api/menus/${this.state.rest_id}`)

      .then(({ data }) =>
        data.filter(obj => obj.menu_type_num === this.state.menu_state)
      )
  ...
  getMenus(rest_id) {
    axios
      // .get(`http://127.0.0.1:9003/api/menus/${rest_id}`)
       //.get(`http://localhost:3005/api/menus/${rest_id}`)
      .get(`http://13.57.201.108:9003/api/menus/${this.state.rest_id}`)
  ...
  retrieveUniqueMenuTypes(menu_data) {
    ...
    for (let i = 0; i < menu_data.length; i++) {
      if (!unique_menus_set.has(menu_data[i]["menu_type_num"])) {
        ...
        unique_menus_dict[menu_data[i]["menu_type_num"]] =
          menu_data[i]["menu_type_name"];
      }
    }
  ...
  render() {
    ...
    let breakfastStyle = this.state.breakfast
      ? "breakfastbutton"
      : "breakfastbutton1";
      ...
      <div>
        <div id="restaurantmenu" className={styles.menucontainer}>
          ...
          <div className={styles.menusectiontitle}>
            <button
              className={styles[breakfastStyle]}
              onClick={e => this.breakfastHandler(e)}
            >
              Breakfast
            </button>
            <button
              className={styles[lunchStyle]}
              onClick={e => this.lunchHandler(e)}
            >
              Lunch
            </button>
            <button
              className={styles[dinnerStyle]}
              onClick={e => this.dinnerHandler(e)}
            >
              Dinner
            </button>
          </div>
          ...
          <MenuSection
            data={this.state.filtered_menu_data.filter(
              obj => obj.menu_section_num === 0
            )}
          />
          <MenuSection
            data={this.state.filtered_menu_data.filter(
              obj => obj.menu_section_num === 1
            )}
          />
          <MenuSection
            data={this.state.filtered_menu_data.filter(
              obj => obj.menu_section_num === 2
            )}
          />

          <div className={styles[viewId]}>
            <div className={styles.viewtext} onClick={() => this.click()}>
              View Full Menu
            </div>
          </div>
        </div>
